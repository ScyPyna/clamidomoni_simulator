...
def train():
    # carica dataset
    data = np.load("dataset/dataset_combined.npy", allow_pickle=True)
    n    = len(data)
    ...
    for epoch in range(300):
        ...
        model.train()
        loss_tr = 0.0
        for rho, r_out, tau in dl_tr:
            rho, r_out, tau = rho.to(device), r_out.to(device), tau.to(device)
            optimizer.zero_grad()
            tau_pred  = model(rho, r_out)
            loss      = mse(tau_pred, tau)
            loss.backward()
        ...
        if loss_val < best_val:
            best_val = loss_val
            torch.save(model.state_dict(), "dataset/best_model_combined.pt")

        if epoch % 20 == 0:
            print(f"Epoch {epoch:3d} | loss_tr={loss_tr:.4f} | loss_val={loss_val:.4f} | lr={scheduler.get_last_lr()[0]:.2e}")

    # ── test ──
    model.load_state_dict(torch.load("dataset/best_model_combined.pt"))
    model.eval()
    tau_true_all, tau_pred_all = [], []
    with torch.no_grad():
        for rho, r_out, tau in dl_te:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("dataset/training_history_combined.png", dpi=120)
