...
data = np.load("dataset_smallring/dataset.npy", allow_pickle=True)
r    = np.load("dataset_smallring/r_centers.npy")
...
for ax, (tau, rhos) in zip(axes, sorted(by_tau.items())):
    ...
    ax.axvline(317, color='gray', linestyle='--', alpha=0.5, label='d_max')
    ax.axvline(562, color='red',  linestyle='--', alpha=0.5, label='d_thr')

plt.tight_layout()
plt.savefig("dataset_smallring/density_check.png", dpi=120)
print("Salvato dataset_smallring/density_check.png")
